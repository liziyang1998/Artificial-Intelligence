...
class Grad_mbgd
{
  ...
  public:
    ...
    //flag代表是否随机
    void Decline(int maxIterator, double Step, int nn);
    void Print();
};

void Grad_mbgd::Init(int a, int b, double c)
{
    n = a;
    m = b;
...
void Grad_mbgd::Decline(int maxIterator, double Step, int nn)
{
    int iterator = 0;
    while (1){
        iterator++; loss = 0;
